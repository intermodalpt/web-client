...
<script>
	...
	$: selectedRoutes = spider
		? Object.keys(spider.routes).map((id) => {
				return routes[id];
		  })
		: [];

	$: score = $stop ? stopScore($stop) : 0.0;
...
{#if $stop}
	<div
		class="p-2 flex flex-row items-center gap-1 lg:min-h-16 w-full text-primary-content bg-primary text-lg font-bold"
	>
		<div class="btn btn-ghost btn-circle sm:mt-0 sm:mb-0" on:click={goBack} on:keypress={goBack}>
			<svg
				class="w-6 h-6"
				xmlns="http://www.w3.org/2000/svg"
				viewBox="0 0 448 512"
				style="fill: hsl(var(--pc))"
			>
				<path
	...
	<div class="overflow-y-scroll overflow-x-hidden">
		{#if $pictures?.length}
			<a>
				<div
					class="bg-cover bg-no-repeat bg-center transition-all hover:scale-110 h-32 lg:h-40"
		...
		<div class="flex flex-col py-2">
			<div class="flex flex-wrap gap-1 pl-2 pr-2 sm:pl-4 sm:pr-3 z-[10000]">
				...
				{/if}
				{#if $stop?.has_visibility_from_within === false}
					<div class="tooltip" data-tip="Difícil visibilidade atempada">
						<img
							class="h-12"
							src="/stopattrs/visible-not.svg"
							alt="Difícil visibilidade atempada"
						/>
					</div>
				{/if}
			</div>
			...
			<RouteListing {selectedRoutes} on:openroute on:openschedule on:openinfo on:hint on:drophint />
			...
			<div class="flex flex-wrap gap-1 pl-2 pr-2 sm:pl-4 sm:pr-3">
				{#if $stop?.has_flag === true}
					<div class="tooltip" data-tip="Identificada">
				...
				{/if}
				{#if $stop?.has_visibility_from_within === true}
					<div class="tooltip" data-tip="Facilmente visível atempadamente">
						<img class="h-12" src="/stopattrs/visible.svg" alt="Facilmente visível atempadamente" />
					</div>
				{/if}
			</div>
			...
			{#if $pictures?.length > 1}
				<div class="grid grid-cols-2">
					{#each $pictures.slice(1, 5) as picture}
						{#if !picture.tagged}
			...
			{:else if $pictures?.length === 1}
				Sem mais fotografias. [Contribuir]
			{:else}
				Sem fotografias.
			{/if}
